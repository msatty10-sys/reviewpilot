...
class PaymentService:
    ...
    @staticmethod
    def create_portal_session(business_id: int, stripe_customer_id: str, return_url: str):
    ...
    def handle_webhook(db: Session, payload: bytes, sig_header: str):
        ...
        if event.type == "checkout.session.completed":
            ...
            PaymentService._update_subscription_from_session(db, session)
        elif event.type in ["customer.subscription.updated", "customer.subscription.deleted"]:
            subscription = event.data.object
            PaymentService._update_subscription_from_stripe(db, subscription)
            
        return event

    @staticmethod
    def _update_subscription_from_session(db: Session, session: stripe.checkout.Session):
    ...
    @staticmethod
    def _update_subscription_from_stripe(db: Session, stripe_sub: stripe.Subscription):
        db_sub = db.query(Subscription).filter(Subscription.stripe_subscription_id == stripe_sub.id).first()
        if db_sub:
            db_sub.status = stripe_sub.status
            db_sub.current_period_end = datetime.fromtimestamp(stripe_sub.current_period_end, tz=timezone.utc)
            db.commit()
